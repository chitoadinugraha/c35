...
fn item_label(item: &ConsumptionItem) -> String {
    let name = item.name.trim();
    let label = if name.is_empty() { item.name_id.trim() } else { name };
    label.replace('|', "/")
}

pub fn food_name_label(items: &[ConsumptionItem], locale: &str) -> String {
    let names: Vec<String> = items.iter().map(item_label).filter(|s| !s.is_empty()).take(2).collect();
    if names.is_empty() {
