...
const server = new McpServer({ name: "c35-inst", version: "0.1.0" });
...
server.registerTool(
  ...
  async ({ scope, kind, enabled, include_deleted }) => {
    ...
    return { content: [{ type: "text", text: JSON.stringify(rows.map(rowToJson), null, 2) }] };
  },
);
...
server.registerTool(
  ...
  async ({ id }) => {
    ...
    return { content: [{ type: "text", text: JSON.stringify(rowToJson(rows[0]), null, 2) }] };
  },
);
...
server.registerTool(
  ...
  async ({ id }) => {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ ok: true, id: instId }) }] };
  },
);
